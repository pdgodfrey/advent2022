...
@rows.each_with_index do |row, i|
  if row.include?("E")
    $start_coordinate = [i, row.index("E")]
  end

  break if $start_coordinate
end
...
class PathStep
  ...
  def initialize(value, y, x, g_score, prev_step)
    ...
    @y = y
    @g_score = g_score
    @h_score = 0# (y - $end_coordinate[0]).abs + (x - $end_coordinate[1]).abs
    @next_steps = []
    @prev_step = prev_step
  end
...
# $all_path_steps = []
#
puts "start : #{$start_coordinate}"
path_step = PathStep.new("E", $start_coordinate[0], $start_coordinate[1], 0,nil )
